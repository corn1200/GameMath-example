...
이 식과 같이 모든 $a$가 0이 아님에도 영벡터를 만들 수 있다면, 선형 결합에 사용된 벡터는 서로 '선형 종속의 관계'를 가진다라고 표현한다.  
따라서 $(1, 1)$과 $(2, 2)$의 두 벡터는 선형 종속인 관계를 갖는다.

반면 영벡터가 나오기 위해서 모든 $a$값이 0이어야 한다면 선형 결합에 사용된 벡터들은 서로 '선형 독립의 관계'를 가진다라고 표현한다.  
...
선형 독립의 관계를 가지는 벡터를 선형 결합하면 벡터 공간에 속한 모든 벡터를 생성할 수 있기 때문이다.    
