...
다음 그림을 예시로 보면, 집합 $X$의 원소 중 3과 4는 대응 관계가 없고 원소 2는 집합 $Y$의 두 원소($B$와 $C$)에 대응된다.     
...
다음 그림은 정의역, 공역, 치역의 개념과 관계를 표현한다.

![정의역, 공역, 치역의 개념](/img/)

또한 ***함수에 사용하는 정의역의 요소를 입력(Input), 입력에 대응하는 공역의 요소를 출력(Output)이라고 한다***.    
이러한 함수의 개념은 다음 그림과 같은 형태로도 표현할 수 있다.
...
다음 그림과 같이 공역과 치역이 동일한 함수다.   
> 참고: 위로의 함수(Onto)라고도 부름

![전사함수의 예시](/img/)

다음 그림은 공역과 치역이 서로 다르므로 전사함수가 아니다.

![전사함수가 아닌 에시](/img/)

## 단사함수
***단사함수(Injection)는 정의역과 공역의 요소가 일대일로 대응되는 함수를 의미한다***.     
다음 그림은 정의역의 모든 요소가 공역의 요소에 일대일로 대응되므로 단사함수다.
> 참고: 일대일 함수(One-to-One)라고도 부름

![단사함수 예시](/img/)

다음 그림은 정의역의 두 요소가 공역의 한 요소에 대응되기 때문에 일대일 대응이 아니므로 단사함수가 아니다.

![단사함수가 아닌 예시](/img/)

## 전단사함수
***전단사함수(Bijection, One-to-One and Onto)는 정의역과 공역의 모든 요소가 빠짐없이 일대일로 대응되는 함수를 의미한다***.    
다음 그림과 같이 전사함수와 단사함수의 두 가지 성질을 모두 만족하면 전단사함수라 할 수 있다.
...
다음 그림과 같이 세 집합 $X, Y, Z$ 사이에 두 함수 $f(x)$와 $g(y)$가 존재하는 상황을 가정해보자.

![합성함수의 예시](/img/)

두 함수 $f(x)$와 $g(y)$를 연쇄적으로 이어 합성함수를 만들면, 다음 그림과 같이 중간에 위치한 집합 $Y$를 생략하고 집합 $X$와 $Z$의 직접적인 대응 관계를 표현할 수 있다.   
...
이번에는 다음 그림과 같이 집합과 함수를 하나씩 더 추가한 대응 관계를 생각해보자.

![4개 집합과 3개 함수로 구성된 대응 관계](/img/)

이 상황에서는 두 합성함수에 대한 경우의 수를 다음 그림의 (a)와 (b)와 같이 생각할 수 있다.

![세 함수로부터 만들어지는 두 합성함수에 대한 경우의 수](/img/)

위 그림의 (a)와 (b)의 두 합성함수가 남은 함수와 다시 합성하는 경우는 각각 다음 그림의 (a)와 (b)와 같이 전개될 것이다.

![합성함수와의 합성 결과](/img/)

위 그림의 (a)와 (b)의 결과는 동일한 대응 관계를 가짐을 볼 수 있다.  
