...
가법성 확인.
- 좌변의 식: $f(\vec{v_1} + \vec{v_2}) = f(x_1 + X_2, y_1 + y_2) \\
= (a(x_1 + x_2) + b(y_1 + y_2), c(x_1 + x_2) + d(y_1 + y_2))$
- 우변의 식: $f(\vec{v_1}) + f(\vec{v_2}) = (ax_1 + by_1, cx_1 + dy_1) + (ax_2 + by_2, cx_2 + dy_2) \\
= (ax_1 + ax_2 + by_1 + by_2, cx_1 + cx_2 + dy_1 + dy_2) \\
\therefore f(\vec{v_1} + \vec{v_2}) = f(\vec{v_1}) + f(\vec{v_2})$

1차 동차성 확인.
- 좌변의 식: $kf(\vec{v}) = (kax + kby, kcx + kdy)$
- 우변의 식: $f(k\vec{v}) = (akx + bky, ckx + dky) \\
